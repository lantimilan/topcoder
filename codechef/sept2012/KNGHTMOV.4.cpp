...
void init()
{
    ...
    for(int i=1; i<=MAX; ++i) {
        comb[i][0] = 1;
        for(int j=1; j<=i; ++j) {
            refadd(comb[i][j], comb[i-1][j-1]);
            refadd(comb[i][j], comb[i-1][j]);
        }
    }
}
...
void solve()
{
    ...
    if (bitcnt[mask]>1)
    {
        ...
        for(int xj=0; memo[mask][xj]>=0; ++xj) {
            int64 curr = 0;
            int j = memo[mask][xj];
            int sub = mask & ~(1<<j);
            //for(int x=0; x<K; ++x) if (sub & 1<<x) 
            for(int xx=0; memo[sub][xx]>=0; ++xx)
            {
                int x = memo[sub][xx];
                ...
                int move = 0;
                move = (int64)dp[sub][x] * getmove(a,b) % MOD;
                curr += move;
            }
            dp[mask][j] = curr%MOD;
        }
...
// strange, even 6000 is not enough
// if each step is nonzero, 500 - (-500) is at most 1000
// so it is at most (2000 choose 1000)?
// comb 5000 not enough?
// why TLE? comb is slow
// TLE, not too bad
//
