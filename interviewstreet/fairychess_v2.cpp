...
inline void refadd(int &a, int b)
{
    a = ((long long)a+b)%MOD;
}

inline void refsub(int &a, int b)
{
    ...
    for(int step=1; step<=M; ++step) {
        ...
        for(int i=1; i<N; ++i) {
            ...
            for(int j=1; j+1<N; ++j) {
                ...
                t+=edge[step][i-1][j+1][0];
                edge[step][i][j][0] = t%MOD;
                t=0;
                if (i-1-S>=0) t+=MOD-sum[step-1][i-1-S][j-1];
                if (j+S<N) t+=sum[step-1][i][j+S];
                t+=edge[step][i-1][j-1][1];
                edge[step][i][j][1] = t%MOD;
            }
        }
        ...
        for(int i=1; i<N; ++i) {
            ...
            /*
                ...
                */
                ...
                t+=sum[step][i-1][j];
                t+=MOD-edge[step][i-1][j][0]; // move of a diamond
                t+=MOD-edge[step][i-1][j][1];
                t+=edge[step][i][j][2];
                t+=edge[step][i][j][3];
                sum[step][i][j]=t%MOD;
            }
