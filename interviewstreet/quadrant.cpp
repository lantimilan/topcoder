...
struct node_t {
    ...
    int begin, end;
    int cnt[4], ops[4];
    int seg[4]; // cnt[] of intersecting with query interval
};

/*
 * speed up from 13s to 12s
...
void uplocal(int id, int op)
{
    ...
    for(int i=0; i<4; ++i) {
        vec[id].cnt[i] = tmp[flip[op][i]]; //tmp[flip(i,op)];
    }        
}
...
void query(int begin, int end, int id, int ans[])
{
    ...
    if (begin > node.end || end < node.begin) { 
        ...
        memset(node.seg, 0, sizeof node.seg);
        return;
    }
    ...
    if (begin<=node.begin && node.end<=end) {
        ...
        memcpy(node.seg, ans, sizeof node.seg);
        return;
    }
    ...
    int tmp[4];
    if (ll<NN) query(begin, end, ll, tmp); for(int i=0; i<4; ++i) ans[i]=tmp[i];
    if (rr<NN) query(begin, end, rr, tmp); for(int i=0; i<4; ++i) ans[i]+=tmp[i];
    memcpy(node.seg, ans, sizeof node.seg);
}

void uphelp(int begin, int end, int id, int op)
{
    ...
    memcpy(tmp, vec[id].seg, sizeof tmp);
    for(int i=0; i<4; ++i) {
        tmp2[i]=tmp[flip[op][i]]; //tmp[flip(i,op)];
    }
    for(int i=0; i<4; ++i) {
        vec[id].cnt[i]-=tmp[i];
        vec[id].cnt[i]+=tmp2[i];
    ...
    if (ll<NN) uphelp(begin, end, ll, op);
    if (rr<NN) uphelp(begin, end, rr, op);
}
...
void update(int begin, int end, int id, int op)
{
    ...
    if (begin <= vec[id].begin && vec[id].end <= end) {        
        // update cnt[] in parent before recurse into children
...
// Quadrant Queries     cpp     2012-02-26 00:02:20     Processed   Accepted    View Details
// #    Status  Signal  Time
// 1    Passed  Success     0.040691
// 2    Passed  Success     0.040561
// 3    Passed  Success     0.04055
// 4    Passed  Success     0.748016
// 5    Passed  Success     0.923277
// 6    Passed  Success     0.394429
// 7    Passed  Success     1.5162
// 8    Passed  Success     0.242787
// 9    Passed  Success     1.30905
// 10   Passed  Success     0.202325
// 11   Passed  Success     2.90196
//
// although quadrant.large will run 11s
//
// Time Limit Exceeded
