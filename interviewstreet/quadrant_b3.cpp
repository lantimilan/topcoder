...
void uplocal(int id, int ops[])
{
    for(int z=0; z<4 && ops[z]>=0; ++z)
    {
    // update ops[]
    int op=ops[z];
    int i;
    ...
    if (i==0) vec[id].ops[0]=op;
        ...
        else if (i==3) vec[id].ops[0]=-1; //memset(vec[id].ops, -1, sizeof vec[id].ops);
    ...
    for(int i=0; i<4; ++i) {
        int target;
        if (opcnt[0]&&opcnt[1]) target=flip[2][i]; //flipXY(i);
        else target=flip[op][i]; //flip(i,op);
...
void query(int begin, int end, int id, int ans[])
{
    ...
    //for(int i=0; i<4 && node.ops[i]>=0; ++i) {
        if (ll<NN) uplocal(ll, node.ops);
        if (rr<NN) uplocal(rr, node.ops);
    //}
...
void update(int begin, int end, int id, int op)
{
    ...
    if (begin <= vec[id].begin && vec[id].end <= end) {        
        ...
        // update locally
        int ops[4]; memset(ops, -1, sizeof ops); ops[0]=op;
        uplocal(id, ops);
        // incr to parents
        //for(int p=id; p; p=(p-1)/2) {
        //    for(int i=0; i<4; ++i)
    ...
    //for(int i=0; i<4 && vec[id].ops[i]>=0; ++i) {
        if (ll<NN) uplocal(ll, vec[id].ops);
        if (rr<NN) uplocal(rr, vec[id].ops);
    //}
...
// speed up
// uplocal is too slow, can do only one update for ops[] from parent
//
// Time Limit Exceeded
