...
{
    vis[r][c][x]=pii(l,start); if (board[r][c]) hits[l][start].insert(pii(r,c));
    int r2, c2;
    r2 = r+dir[x][0];
    c2 = c+dir[x][1];
    // bounce
    ...
    if (vis[r2][c2][x].first<0) dfs(l,start,r2,c2,x);
}
...
int main()
{
    ...
    for(int l=0; l<K; ++l)
    {
        for(int x=0; x<4; ++x)
        {
            ...
            if (vis[i][j][x].first <0) dfs(l,x,i,j,x);
            else {
                pii p=vis[i][j][x];
                int id=p.first;
                int dd=p.second;
