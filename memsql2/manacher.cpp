...
/*
    ...
    */
    ...
    manacher2(s, mypal);
    int pos = -1;
    for (int p = 0; p < 2*s.length(); p += 2) {
        if (mypal[p] >= 100) { pos = p; break; }
    }
    string ans;
    if (pos >= 0) {
        ans = s.substr(pos/2 - 100/2, 100);
    } else {
        int pmax = 0;
        for (int p = 1; p < 2 * s.length(); p++) {
            if (mypal[p] > mypal[pmax]) pmax = p;
        }
        int size = mypal[pmax];
        ans = s.substr(pmax/2 - size/2, size);
    }
    cout << ans << endl;
}
